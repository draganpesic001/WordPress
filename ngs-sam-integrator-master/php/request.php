	...
	if( 0 == $samerr ) {    
	    ...
	    if ( !empty( $request_file_handle ) ) {
			...
			if ( !empty( $request_response_xml ) ) {
			    ...
			    $xml = simplexml_load_string($request_response_xml);
			    
			    $request_status_code = $xml->status->code;
			    $request_status_message = $xml->status->message;
			    $request_status_id = $xml->status->requestID;
			    //echo "code=";echo $request_status_code; echo ", message="; echo $request_status_message; echo ", id="; echo $request_status_id;
			    /*
			    $request_xml_array = XMLtoArray($request_response_xml);
			    echo "request xml array: "; print_r($request_xml_array); echo "<br>";
			    $request_result = Keys2Lower($request_xml_array['REQUEST']);
			    echo "request result: ". $request_result."<br>";
			    ...
			    */
			    if ( empty( $request_status_code ) ) $samerr = 804;
				else if( $request_status_code == 705 ) $samerr = 705;
				else if( $request_status_code == 708 ) $samerr = 708;
			} else {
				$samerr = 804;
			}
	    } else { 
...
function ngs_sam_error( $code )
{
    ...
    switch( $code )
    {
    case 705 : $request_status_message = "Request limit reached. You can only request 10 songs per day"; break;
    case 708 : $request_status_message = "You have already requested this song and it is waiting in the request queue to be played."; break;
